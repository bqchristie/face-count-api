...
describe("/api/v1/image/:id", () => {
  test("getting a single result succeeds for authorized user", async () => {
    ...
    expect(res.body).toEqual(data);
    expect(res.status).toBe(200);
    expect(ImageService.get).toHaveBeenCalledWith(1);
  });

  test("getting a single result fails for anonymous user", async () => {
    const req = supertest(app);
    const res = await req.get("/api/v1/image/1");
    expect(res.status).toBe(401);
  });
  ...
  test("Image update", async () => {
    ...
    expect(res.body).toEqual(data);
    expect(res.status).toBe(200);
    expect(ImageService.update).toHaveBeenCalledWith(1, data);
  });
  ...
  test("Image deletion", async () => {
    ...
    expect(ImageService.delete).toHaveBeenCalledWith(1);
  });
});
